    ...
    <div class="mb-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <?php foreach ($all_media as $item) : ?>
                ...
                <?php else : ?>
                    <div class="relative group overflow-hidden rounded-lg cursor-pointer bg-gray-900" onclick="openVideoModal('<?php echo esc_attr($item['youtube_id'] ?: $item['vimeo_id']); ?>', '<?php echo esc_attr($item['youtube_id'] ? 'youtube' : 'vimeo'); ?>', '<?php echo esc_attr($item['title']); ?>')">
                        ...
                        <div class="absolute inset-0 bg-black/40 flex items-center justify-center">
                            <div class="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm group-hover:bg-white/30 transition">
                                <svg class="w-8 h-8 text-white mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M6.3 2.841A1.5 1.5 0 004 4.11V15.89a1.5 1.5 0 002.3 1.269l9.344-5.89a1.5 1.5 0 000-2.538L6.3 2.84z"/>
                                </svg>
                            </div>
                        </div>
                        <?php if (!empty($item['title'])) : ?>
                            <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                                <p class="text-white font-bold text-sm"><?php echo esc_html($item['title']); ?></p>
                            </div>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
