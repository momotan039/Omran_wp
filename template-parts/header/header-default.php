        ...
        <div class="flex justify-between items-center h-20">
            ...
            <a href="<?php echo esc_url(home_url('/')); ?>" class="flex items-center gap-2">
                ...
                <?php else : ?>
                    ...
                    <div class="w-10 h-10 bg-gradient-to-br from-blue-600 to-violet-600 rounded-xl flex items-center justify-center text-white font-bold text-2xl shadow-lg">
                        <?php echo esc_html($logo_first_char); ?>
                    </div>
            ...
            <div class="hidden md:flex items-center gap-8">
                ...
                <div class="flex items-center gap-4 mr-4">
                    <?php
                    // Get pricing page link
                    $pricing_page = get_page_by_path('pricing');
                    if ($pricing_page) {
                        echo '<a href="' . esc_url(get_permalink($pricing_page)) . '" class="bg-blue-600 text-white px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-blue-700 transition-all shadow-md shadow-blue-200">ابدأ الآن</a>';
                    }
                    ?>
                </div>
            </div>
    ...
    <div class="md:hidden bg-white border-t border-slate-100 p-4 absolute top-full left-0 right-0 shadow-xl tech-mobile-menu" style="display: none;">
        <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-3 pt-4">
                ...
                $pricing_page = get_page_by_path('pricing');
                if ($pricing_page) {
                    echo '<a href="' . esc_url(get_permalink($pricing_page)) . '" class="text-center py-3 text-white font-semibold bg-blue-600 rounded-xl">ابدأ الآن</a>';
                }
                ?>
            </div>
        </div>
